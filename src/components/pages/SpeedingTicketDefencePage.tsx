...
export default function SpeedingTicketDefencePage() {
  ...
  const honestFAQs = [
    ...
    { question: "What outcomes are realistic?", answer: "Every case is decided on its own evidence, so we can't guarantee results. Realistic outcomes for a contested speeding charge include: reduction to a zero-point offence (a common negotiated result where the Crown's case is otherwise solid), withdrawal (where the officer is unavailable or radar/laser documentation is incomplete), acquittal at trial, or conviction-as-charged. Higher speeds are harder to reduce but reduction remains possible with a clean driving record or evidentiary issues." },
