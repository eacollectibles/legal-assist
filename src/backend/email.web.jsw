...
async function loadGraphConfig() {
  try {
    ...
    return { tenantId, clientId, clientSecret, businessEmail };
  } catch (error) {
...
async function getAccessToken(config) {
  ...
  cachedToken = {
    ...
    expiresAt: Date.now() + (Number(data.expires_in || 0) - 60) * 1000, // subtract 60s margin
  };
...
export async function sendEmail(options) {
  ...
  try {
    ...
    // sendMail returns 202 Accepted with no body on success; capture headers for traceability
    ...
    return {
      success: true,
      timestamp,
      headers: { requestId, clientRequestId, agsDiagnostic },
    };
  } catch (error) {
...
export async function testGraphConnection() {
  try {
    ...
    const response = await fetch(
      `https://graph.microsoft.com/v1.0/users/${encodeURIComponent(config.businessEmail)}`,
...
/**
 * Backend function to get business email (without exposing other secrets)
 */
export async function getBusinessEmail() {
  try {
    const businessEmail = await getSecret('BUSINESS_EMAIL');
    return { success: true, email: businessEmail };
  } catch (error) {
    return { success: false, error: 'Failed to retrieve business email' };
  }
}
